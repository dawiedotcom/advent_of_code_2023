...
int count_words(text_image<char>& input, unordered_set<word_t>& words, int len=1) {
  ...
  if (len == xmas.size()+1) {
    ...
    for (const auto word1 : words) {
      for (const auto word2 : words) {
        if (word1 == word2)
          continue;
        if ((word1.prev_end_x == word2.prev_end_x)
            && (word1.prev_end_y == word2.prev_end_y) 
            && (word1.direction.size() == 2)
            && (word2.direction.size() == 2)) {
          result ++;
        }
      }
    }
  ...
  if (len == 1) {
    for (size_t i=0; i<input.X(); i++) {
      for (size_t j=0; j<input.Y(); j++) {
        if (input[i, j] == xmas[0]) {
          for (const string dir : word_t::directions)
            words.emplace(i, j, i, j, 1, dir, xmas.substr(0, 1));
        }
      }
    }
  ...
  for (const word_t word : words) {
    ...
    for (const auto [dir, neighbor] : neighbors) {
      if (dir == word.direction && *neighbor == xmas[len-1]) {
        ...
        new_words.emplace(word.end_x, word.end_y, new_end_x, new_end_y, len, dir, word.w + xmas[len-1]);
      }
    }
  }
